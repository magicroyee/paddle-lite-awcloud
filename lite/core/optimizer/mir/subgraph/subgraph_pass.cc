      ...
      CHECK(out_n->IsArg());
        for(auto* tmp_op: out_n->outlinks) {
          ...
          if(tmp_op_info->HasAttr("enable_int8") ||
             tmp_op_info->HasAttr("forced_scale")) {
            is_next_quantized = true;
          } else {
            is_next_quantized = false;
            break;
          }
        }
    }
    bool is_quantized = op_info->HasAttr("forced_scale") ||
                        op_info->HasAttr("enable_int8");
    auto place = stmt.place();
    return (supported_lists.count(stmt.op_type()) != 0 &&
           is_quantized) && (is_quantized && is_next_quantized);
  };
  auto subgraph_partition_configs = ReadSubgraphPartitionConfigsFromEnv();
  SubgraphFuser fuser(graph.get(),
                      teller,
